...
public extension Z3Context {
    ...
    func makeEmptySet<T: SortKind>(_ domain: T.Type) -> Z3Set<T> {
        return Z3Ast(context: self, ast: Z3_mk_empty_set(context, domain.getSort(self).sort))
    }
    ...
    func makeSetAdd<T: SortKind>(_ set: Z3Set<T>, element: Z3Ast<T>) -> Z3Set<T> {
        return Z3Ast(context: self, ast: Z3_mk_set_add(context, set.ast, element.ast))
    }
    ...
    func makeSetDel<T: SortKind>(_ set: Z3Set<T>, element: Z3Ast<T>) -> Z3Set<T> {
        return Z3Ast(context: self, ast: Z3_mk_set_del(context, set.ast, element.ast))
    }
    ...
    func makeSetUnion<T: SortKind>(_ sets: [Z3Set<T>]) -> Z3Set<T> {
        ...
        return Z3Ast(context: self, ast: Z3_mk_set_union(context, UInt32(sets.count), sets))
    }

    /// Take the union of a list of sets
    ///
    /// Type-erased version
    func makeSetUnionAny(_ sets: [AnyZ3Ast]) -> AnyZ3Ast {
        let sets = sets.toZ3_astPointerArray()
    ...
    func makeSetDifference<T: SortKind>(_ arg1: Z3Set<T>, _ arg2: Z3Set<T>) -> Z3Set<T> {
        return Z3Ast(context: self, ast: Z3_mk_set_difference(context, arg1.ast, arg2.ast))
    }
